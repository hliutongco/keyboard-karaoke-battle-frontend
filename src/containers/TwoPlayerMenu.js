...
class TwoPlayerMenu extends React.Component {
  state = {
    display: "",
    selection: "",
    video: "",
  ...
  handleKeydown = (event) => {
    ...
    if(event.key === "Enter" && this.state.display){
      ...
      this.setState({selection: lyricsArr[index]});
    }
  ...
  render() {
    if(this.state.selection === 'roar'){
      window.removeEventListener("keydown", this.handleKeydown);
      return <Redirect to='/rooms/roar'/>
    }
    else if (this.state.selection === 'lookAtMeNow') {
      window.removeEventListener("keydown", this.handleKeydown);
      return <Redirect to='/rooms/lookAtMeNow'/>
    }
...
export default connect(null, mapDispatchToProps)(TwoPlayerMenu)
